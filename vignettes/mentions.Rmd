...
people <- count_lenin_mentions(words = top_proper_nouns$people[1:15])
...
parties <- ggtextcounts::count_mentions_in_dataframe(
  dataframe = lenin_with_adjustments,
  words = c(
...
```{r}
kwords_a <- lenin %>%
  mutate(nw = stringr::str_count(text, "\\S+")) %>%
  group_by(year) %>%
  summarize(annualwords = sum(nw)/1000)
```
...
party_graph_df  %>%
  ...
  aes(x = year, y = n) +
  geom_hline(yintercept = 0, color = "grey40") +
  geom_vline(xintercept = 1903, color = "grey70", linetype = 'dashed') +
  geom_bar( 
    stat = "identity",
    #fill = "grey60",
    aes(fill = year == 1917),
  ...
  labs(
    ...
    Dashed line: RSDLP split into Bolshevik/Menshevik factions (1903).
...
party_graph_df  %>%
  ...
  labs(
    title = "Lenin & the rise and fall of political parties",
    subtitle = "Number of mentions of select political parties (per 1000 words)",
    caption = "Red bar: 1917 February/October Revolutions.
    Dashed line: RSDLP split into Bolshevik/Menshevik factions (1903).
    Includes synonyms (e.g., Bolshevism).
    "
  ) +
  scale_y_continuous(expand = c(0.0, 0., 0.2, 0.2), breaks = c(0:5)) +
  scale_x_continuous(breaks = seq(1895, 1920, 5)) +
  scale_fill_manual(
    values = c("grey60", "#f75d6f")
  )
...
parties %>%
  ...
  group_by(year, abbrev, word, title) %>%
  summarize(n = sum(mentions))%>%
  arrange(desc(n))
